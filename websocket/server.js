...
var hyperactiv = require('hyperactiv');
var handlers = require('hyperactiv/handlers');
...
function server(wss) {
    wss.host = (data, options) => {
        const obj = hyperactiv.observe(data || { }, options || { deep: true, batch: true, bubble: true });
    ...
    ws.on('message', msg => {
        ...
        if(msg.type == 'sync') {
            if(obj && typeof obj === 'function') {
                obj = hyperactiv.observe(obj(msg.value), { deep: true, batch: true });
            } else if(obj == null) obj = hyperactiv.observe(msg.value, { deep: true, batch: true });
